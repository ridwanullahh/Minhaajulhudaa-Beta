# Deployment Guide — Minhaajulhudaa (school/charity/travels suite)

> BismiLLAH Ar-Rahman Ar-Raheem. This document is the developer contract for deploying
> Minhaajulhudaa. It MUST stay accurate for both supported targets.
...
- Build: `npm run build`
- Output dir: `dist/ (see `scripts/prepare-pages-deploy.mjs` / `scripts/build-pages.sh`)`
- Deploy (Direct Upload, preferred — does not consume the 500 builds/month CI budget):
  `npx wrangler pages deploy dist/ (see `scripts/prepare-pages-deploy.mjs` / `scripts/build-pages.sh`) --project-name=minhaajulhudaa-beta`
- Pages **Functions** policy: API routes compile to Pages Functions; `scripts/patch-wrangler.mjs` aligns the Pages config. Content routes static.
...
- Seeds: `scripts/create-collections.mjs` then `scripts/seed-new-lightbase.mjs`
  (or per-vertical `scripts/seed/{school,charity,travels,masjid}.mjs`).
