...
/**
 * Load seed data.
 * Uses bundled TS file (src/lib/seed-data.ts) which is generated by
 * scripts/bundle-data.mjs from the JSON files in /data.
 * This approach is Cloudflare edge-compatible (no filesystem access needed).
 */
export async function loadSeedData(): Promise<Record<string, any[]>> {
  try {
    const { seedData } = await import('./seed-data');
    console.log(`[local-db] Loaded ${Object.keys(seedData).length} collections from bundled seed data`);
    return seedData;
  } catch (err) {
    console.error('[local-db] Failed to load bundled seed data:', err);
    return {};
  }
}
