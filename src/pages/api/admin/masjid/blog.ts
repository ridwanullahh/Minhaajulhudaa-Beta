...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    if (id) {
      const doc = await lightbase.getById('masjid_blog_posts', id);
      return new Response(JSON.stringify(doc?.document || {}), { headers: { 'Content-Type': 'application/json' } });
    }
    const result = await lightbase.query('masjid_blog_posts', { sort: 'publishedAt:desc', limit: 100 });
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const result = await lightbase.insert('masjid_blog_posts', body);
...
export const PATCH: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    const result = await lightbase.update('masjid_blog_posts', id, body);
...
export const DELETE: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    await lightbase.delete('masjid_blog_posts', id);
