...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    if (id) {
      const doc = await lightbase.getById('school_quizzes', id);
      return new Response(JSON.stringify(doc?.document || {}), { headers: { 'Content-Type': 'application/json' } });
    }
    const result = await lightbase.query('school_quizzes', { sort: 'title:asc', limit: 100 });
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const result = await lightbase.insert('school_quizzes', body);
...
export const PATCH: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    const result = await lightbase.update('school_quizzes', id, body);
...
export const DELETE: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    await lightbase.delete('school_quizzes', id);
