...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    if (id) {
      const doc = await lightbase.getById('school_courses', id);
      return new Response(JSON.stringify(doc?.document || {}), { headers: { 'Content-Type': 'application/json' } });
    }
    const result = await lightbase.query('school_courses', { sort: 'title:asc', limit: 100 });
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const result = await lightbase.insert('school_courses', body);
...
export const PATCH: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    const result = await lightbase.update('school_courses', id, body);
...
export const DELETE: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    await lightbase.delete('school_courses', id);
