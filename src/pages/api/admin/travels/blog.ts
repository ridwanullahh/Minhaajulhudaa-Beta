...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    if (id) {
      const doc = await lightbase.getById('travels_blog_posts', id);
      return new Response(JSON.stringify(doc?.document || {}), { headers: { 'Content-Type': 'application/json' } });
    }
    const result = await lightbase.query('travels_blog_posts', { sort: 'publishedAt:desc', limit: 100 });
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    body.authorEmail = body.authorEmail || getSessionCookieValue(request)?.email;
    body.createdAt = new Date().toISOString();
    body.updatedAt = new Date().toISOString();
    if (!body.publishedAt && body.status === 'published') {
      body.publishedAt = new Date().toISOString();
    }
    const result = await lightbase.insert('travels_blog_posts', body);
...
export const PATCH: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    const result = await lightbase.update('travels_blog_posts', id, body);
...
export const DELETE: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    await lightbase.delete('travels_blog_posts', id);
