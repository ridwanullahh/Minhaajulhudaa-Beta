...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    if (id) {
      const doc = await lightbase.getById('travels_packages', id);
      return new Response(JSON.stringify(doc?.document || {}), { headers: { 'Content-Type': 'application/json' } });
    }
    const result = await lightbase.query('travels_packages', { sort: '_created_at:desc', limit: 100 });
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const result = await lightbase.insert('travels_packages', body);
...
export const PATCH: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    const result = await lightbase.update('travels_packages', id, body);
...
export const DELETE: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    await lightbase.delete('travels_packages', id);
